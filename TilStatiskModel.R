...
CreateMatrixes1 <- function(data,StartDate,EndDate,round) {
  ...
  #Finder holdene der er i Superligaen i indevÊrende sÊson
  dataUNan <- na.omit(data1)
  #detach(data1)
  #sÊtter Y-kontingenstabellen op:
...
varbeta <- n$KV[-11,-11]#varians pÂ betaer
sqrt(diag(varbeta))#Standardfejl pÂ betaer
t(x[,1])%*%beta #log(AGFs styrke)
vlpi1<- t(x[,1])%*%varbeta%*%x[,1]#Varians pÂ log(AGF)
sqrt(diag(vlpi1))#Standardfejl pÂ log(AGF)
piagf <- exp(x[,1]%*%beta);piagf#AGFs styrke
varagf <- exp(t(x[,1])%*%beta)%*%(t(x[,1])%*%varbeta%*%x[,1])%*%exp(t(x[,1])%*%beta);varagf#Varians pÂ AGFs styrke
sfagf <-sqrt(varagf);sfagf#standardfejl pÂ AGFs styrke
...
#sqrt(diag(n$KV))
varbeta <- n$KV[-11,-11]#varians pÂ betaer
sfb <- sqrt(diag(varbeta));names(sfb)=names(beta)#Standardfejl pÂ betaer
round(beta,3)
round(sfb,3)
#t(x[,1])%*%beta #log(AGFs styrke)
#vlpi1<- t(x[,1])%*%varbeta%*%x[,1]#Varians pÂ log(AGF)
#sfls <-sqrt(diag(vlpi1));sfls#Standardfejl pÂ log(AGF)
#varpi <- exp(t(x[,1])%*%beta)%*%(t(x[,1])%*%varbeta%*%x[,1])%*%exp(t(x[,1])%*%beta);varagf#Varians pÂ AGFs styrke
#sfagf <-sqrt(varagf);sfagf#standardfejl pÂ AGFs styrke
#exp(t(x[,1])%*%beta)*sfls
